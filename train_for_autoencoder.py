...
def show_history(history, out_folder):
    ...
    #L2
    plt.figure()   #新しいウィンドウを描画
    plt.plot(history["train_L2_loss"], label = "train L2 loss")
    plt.plot(history["val_L2_loss"], label = "val L2 loss")
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
    plt.xticks(xticks_range)
    # plt.ylim(0, 1.2)
    plt.savefig(output_folder_grah+'/L2_loss.png')
...
def uni_train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False, data_flag="IEMOCAP", modal="t", kl_loss=None):
    ...
    avg_accuracy = round(accuracy_score(labels,preds, sample_weight=masks)*100, 2)
    avg_fscore = round(f1_score(labels,preds, sample_weight=masks, average='weighted')*100, 2)
    avg_kl_loss = round(np.sum(kl_losses)/(len(losses)*np.sum(masks)), 4)
    avg_l2_loss = round(np.sum(l2_losses)/(len(losses)*np.sum(masks)), 4)
...
if __name__ == '__main__':
    ...
    history = {"train_loss": [], "train_acc": [],
            "val_loss": [], "val_acc": [],
            "train_kl_loss": [], "train_L2_loss": [],
            "val_kl_loss": [], "val_L2_loss": [],
            "lr": []
            }
    ...
    if args.autoencoder == "symmetry":
        ...
        temp = args.temp
        finetune_flag = False #True ファインチューニングなし
        model = Symmetry_AutoEncoder(learned_model, hidden_dim, latent_dim, dropout, n_classes, temp, finetune_flag)
    else:
        #学習済みモデルのパス
        learned_model = args.learned_model
        hidden_dim=args.hidden_dim
        ...
        #温度係数　IEMOCAP 1 MELD 8
        temp = args.temp
        model = AutoEncoder(learned_model, hidden_dim, latent_dim, dropout, n_classes, temp)
    ...
    for e in range(n_epochs):
        ...
        train_kl, train_l2,\
            = uni_train_or_eval_model(model, loss_function, train_loader, e, optimizer, True, data_flag=args.Dataset, modal=args.model, kl_loss=kl_loss)

        valid_loss, valid_acc, _, _, _, valid_fscore, hoge, _,\
        _, _,\
            = uni_train_or_eval_model(model, loss_function, valid_loader, e, data_flag=args.Dataset, modal=args.model, kl_loss=kl_loss)

        #検証，テスト(テストデータを使用)
        test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, misclassified, all_pred,\
        test_kl, test_l2,\
            = uni_train_or_eval_model(model, loss_function, test_loader, e, data_flag=args.Dataset, modal=args.model, kl_loss=kl_loss)
        ...
        history['train_L2_loss'].append(train_l2)
        history['val_kl_loss'].append(test_kl)
        history['val_L2_loss'].append(test_l2)
