...
def train_or_eval_model(model, loss_function, kl_loss, ws_loss, dataloader, epoch, optimizer=None, train=False, gamma_1=1.0, gamma_2=1.0, gamma_3=1.0, data_flag="IEMOCAP"):
    ...
    for data in dataloader:
        ...
        if data_flag == "MELD_c":
            textf, visuf, acouf, qmask, umask, label = [d.cuda() for d in data[:-3]] if cuda else data[:-3]
        ...
        # print("cmatrix:", np.shape(cmatrix[0]))
        # print('vids:', vids)
        ...
        if train == False:
            for xi in range(x):
                ...
                for yi in range(y):
                    ...
                    if mask_value == 1:
                        character = cmatrix[xi][yi].index(1)
        ...
        # for x in range(umask[0]):
        #     for y in range(umask[1]):
        ...
        #                 character = cmatrix[conv_idx][i].index(1) 
        #                 all_pred.append({"vid": vid,
        #                         "utt_index": i,
        #                         "character": character,
...
if __name__ == '__main__':
    ...
    for e in range(n_epochs):
        ...
        train_ce_loss_t, train_ce_loss_v, train_ce_loss_a, \
        train_kl_loss_t, train_kl_loss_v, train_kl_loss_a, \
        train_ws_loss_t, train_ws_loss_v, train_ws_loss_a= train_or_eval_model(model, loss_function, kl_loss, ws_loss, train_loader, e, optimizer, True, data_flag=args.Dataset)
        ...
        test_ws_loss_t, test_ws_loss_v, test_ws_loss_a= train_or_eval_model(model, loss_function, kl_loss, ws_loss, test_loader, e, data_flag=args.Dataset)
    ...
    with open(args.out_path+"/misclassified.csv", "w", newline='') as f:
        fieldnames = ["vid", "utt_index", "character","pred", "true", "text"]
    ...
    # Save all_pred examples
    with open(args.out_path+"/all_pred.csv", "w", newline='') as f:
        fieldnames2 = ["vid", "utt_index", "character","pred", "true", "text"]
