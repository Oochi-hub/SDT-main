...
from model import MaskedNLLLoss, MaskedKLDivLoss, Transformer_Based_Model, WassersteinLoss, LogWassersteinLoss
...
# 学習曲線のプロット
def show_history(history, out_folder, data):
...
def train_or_eval_model(model, loss_function, kl_loss, ws_loss, dataloader, epoch, optimizer=None, train=False, gamma_1=1.0, gamma_2=1.0, gamma_3=1.0):
    ...
    for data in dataloader:
        ...
        """
        各入力の説明とデモ(21:バッチ内最大発話数, 8:バッチサイズ 9:MELDの最大話者 IEMOは2)
        textf: テキスト特徴量           torch.Size([21, 8, 1024])
        visuf: ビジュアル特徴量         torch.Size([21, 8, 342])
        acouf: オーディオ特徴量         torch.Size([21, 8, 300])
        qmask: 人物の区別のための行列   torch.Size([8, 21, 9])
        umask: 発話のパディングマスク   torch.Size([8, 21])
        label: 教師ラベル               torch.Size([8, 21])
        """
        ...
        ws_t_prob, ws_a_prob, ws_v_prob, ws_all_prob = model(textf, visuf, acouf, umask, qmask, lengths)
        ...
        utt_lens = [(umask[j] == 1).nonzero().size(0) for j in range(len(umask))]
        utt_base = [0] + list(np.cumsum(utt_lens[:-1]))  # 各会話のb_idx開始位置

        if train == False:
            for conv_idx, (vid, utt_len, base) in enumerate(zip(vids, utt_lens, utt_base)):
                for i in range(utt_len):
                    global_idx = base + i
                    if umask.view(-1)[global_idx] == 1:
                        pred_label = int(pred_[global_idx])
                        true_label = int(labels_[global_idx])
                        if pred_label != true_label:
                            misclassified.append({
                                ...
                                "actual": true_label,
                                "text": texts[conv_idx][i] if i < len(texts[conv_idx]) else "[INDEX ERROR]"
                            })
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore, misclassified, \
...
if __name__ == '__main__':
    ...
    # M = torch.ones(n_classes, n_classes) - iemo_matrix
    ...
    for e in range(n_epochs):
        ...
        #学習
        train_loss, train_acc, _, _, _, train_fscore, hoge, \
        train_ce_loss, train_kl_loss, train_ws_loss, train_task_loss, \
        train_ce_loss_t, train_ce_loss_v, train_ce_loss_a, \
        train_kl_loss_t, train_kl_loss_v, train_kl_loss_a, \
        train_ws_loss_t, train_ws_loss_v, train_ws_loss_a= train_or_eval_model(model, loss_function, kl_loss, ws_loss, train_loader, e, optimizer, True)

        valid_loss, valid_acc, _, _, _, valid_fscore, hoge, \
        valid_ce_loss, valid_kl_loss, _, _, \
        _, _, _, \
        _, _, _, \
        _, _, _= train_or_eval_model(model, loss_function, kl_loss, ws_loss, valid_loader, e)

        #検証，テスト(テストデータを使用)
        test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, misclassified,\
        ...
        test_ws_loss_t, test_ws_loss_v, test_ws_loss_a= train_or_eval_model(model, loss_function, kl_loss, ws_loss, test_loader, e)
        ...
        #     GAMMA_2 += 0.2
        #     GAMMA_3 += 0.2
        #     print("Corrent Decay:", (GAMMA_2, GAMMA_3))
        ...
        # if (e+1) == args.kl_epoch:
        #     GAMMA_1 = args.gannma_1
        #     GAMMA_2 = args.gannma_2
        #     GAMMA_3 = args.gannma_3
        #     print("Corrent Decay:", GAMMA_1, GAMMA_2, GAMMA_3)

        #DECAY_CE_T, DECAY_CE_A, DECAY_CE_V, DECAY_KL_T, DECAY_KL_A, DECAY_KL_V = lst
        # if (e+1) == args.kl_epoch:
        #     GAMMA_1 = args.gannma_1
        #     DECAY_CE_V = args.gannma_2
        #     DECAY_KL_V = args.gannma_3

        # if args.flag_1:
        #     GAMMA_1 = GAMMA_1 * 1.02
        # if args.flag_2:
        #     GAMMA_2 = GAMMA_2 * 1.02
        # if args.flag_3:
        #     GAMMA_3 = GAMMA_3 * 1.02
        # print("Corrent Decay:", GAMMA_1, GAMMA_2, GAMMA_3)
    ...
    plt.savefig(args.out_path+'/cm.png')

    # Save misclassified examples
    with open(args.out_path+"/misclassified.csv", "w", newline='') as f:
        fieldnames = ["vid", "utt_index", "predicted", "actual", "text"]
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for row in best_misclassified:
            filtered_row = {key: row.get(key, "") for key in fieldnames}
            writer.writerow(filtered_row)

        print("Saved misclassified examples to misclassified.csv")

    #学習の記録
    show_history(history, args.out_path, args.Dataset)
