...
from model import MaskedNLLLoss, MaskedKLDivLoss_2, autoencoder_and_multimodal_Model_with_flag
...
def show_history(history, out_folder):
    ...
    plt.savefig(output_folder_grah+'/kl_loss.png')
    ...
    plt.legend()
    plt.xticks(xticks_range)
    plt.savefig(output_folder_grah+'/train_kl_modal_loss.png')
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False, data_flag="IEMOCAP", kl_loss=None, merged_df=None):

    #表示用の入れ子
    losses, preds, labels, masks, = [], [], [], []
    kl_losses, ce_losses = [], []
    kl_losses_t, kl_losses_a, kl_losses_v = [], [], []
    misclassified = []
    all_pred = []
    ...
    for data in dataloader:
        ...
        #tensorに変換
        batch_allprob_t = torch.from_numpy(batch_allprob_t.astype(np.float32)).clone()
        batch_allprob_a = torch.from_numpy(batch_allprob_a.astype(np.float32)).clone()
        batch_allprob_v = torch.from_numpy(batch_allprob_v.astype(np.float32)).clone()
        ...
        if demo_charaID_flag:
            ...
            final_prob, final_log_prob, kl_log_prob_t, kl_log_prob_a, kl_log_prob_v = model(textf, visuf, acouf, umask, qmask, lengths, modality_flags=batch_modal_flag)
        ...
        #KL損失計算
        target_t = batch_allprob_t.view(-1, batch_allprob_t.size()[2]).cuda()
        target_a = batch_allprob_a.view(-1, batch_allprob_a.size()[2]).cuda()
        target_v = batch_allprob_v.view(-1, batch_allprob_v.size()[2]).cuda()

        kl_lp_t = kl_log_prob_t.view(-1, kl_log_prob_t.size()[2])
        kl_lp_a = kl_log_prob_a.view(-1, kl_log_prob_a.size()[2])
        kl_lp_v = kl_log_prob_v.view(-1, kl_log_prob_v.size()[2])

        #親表現と子表現のKL情報量誤差用
        L_kl_t = kl_loss(kl_lp_t, target_t, mask_t, umask)
        L_kl_a = kl_loss(kl_lp_a, target_a, mask_a, umask)
        L_kl_v = kl_loss(kl_lp_v, target_v, mask_v, umask)
        # L_kl_t = kl_loss(ce_log_final_prob, target_t, mask_t, umask)
        # L_kl_a = kl_loss(ce_log_final_prob, target_a, mask_a, umask)
        # L_kl_v = kl_loss(ce_log_final_prob, target_v, mask_v, umask)

        DECAY_KL_T, DECAY_KL_A, DECAY_KL_V = 1.0, 1.0, 1.0

        L_kl   = DECAY_KL_T * L_kl_t + DECAY_KL_A * L_kl_a + DECAY_KL_V * L_kl_v

        ##モデル全体の損失
        L_task = ALPHA * L_ce + (1-ALPHA) * L_kl #* 10000
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore, misclassified, all_pred,\
           avg_kl_loss, avg_ce_loss,\
           avg_kl_loss_t, avg_kl_loss_a, avg_kl_loss_v
...
if __name__ == '__main__':
    ...
    history = {"train_loss": [], "train_acc": [],
            "val_loss": [], "val_acc": [],
            "train_kl_loss": [], "train_ce_loss": [],
            "val_kl_loss": [], "val_ce_loss": [],
            "lr": [],
            "train_kl_loss_t": [], "train_kl_loss_a": [], "train_kl_loss_v": []
            }
    ...
    model = autoencoder_and_multimodal_Model_with_flag(auto_t, auto_a, auto_v, hidden_dim, latent_dim, n_classes, finetune_flag, temp)
    ...
    if args.modal_mask_type == "correct":
        ...
        df_train = pd.read_csv("binary_data/binary/all_pred_merged_train_correct.csv")
    elif args.modal_mask_type == "good_at":
        print("学習データにおける正答かつ得意ラベルサンプルのみ蒸留")
        df_train = pd.read_csv("binary_data/binary/all_pred_merged_train_good_at.csv")
    ...
    for e in range(n_epochs):
        ...
        train_kl, train_ce,\
        train_kl_t, train_kl_a, train_kl_v,\
            = train_or_eval_model(model, loss_function, train_loader, e, optimizer, True, data_flag=args.Dataset, kl_loss=kl_loss, merged_df=merged_df_train)


        #検証，テスト(テストデータを使用)
        test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, misclassified, all_pred,\
        test_kl, test_ce,\
        test_kl_t, test_kl_a, test_kl_v,\
            = train_or_eval_model(model, loss_function, test_loader, e, data_flag=args.Dataset, kl_loss=kl_loss, merged_df=merged_df_test)
        ...
        history['val_kl_loss'].append(test_kl)
        history['val_ce_loss'].append(test_ce)
        history["lr"].append(optimizer.param_groups[0]['lr'])  # 現在の学習率を記録
        history['train_kl_loss_t'].append(train_kl_t)
        history['train_kl_loss_a'].append(train_kl_a)
        history['train_kl_loss_v'].append(train_kl_v)
