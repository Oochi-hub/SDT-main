...
def train_or_eval_model(model, loss_function, kl_loss, ws_loss, dataloader, epoch, optimizer=None, train=False, data_flag="IEMOCAP", loss_func="kl", source_modal="text"):
    ...
    for data in dataloader:
        ...
        #モダリティ補間実験用
        ##各モダリティの0ベクトルを作成
        ##ソースモダリティ以外の入力を0ベクトルに置き換える．
        textf_zero = torch.zeros_like(textf)
        visuf_zero = torch.zeros_like(visuf)
        acouf_zero = torch.zeros_like(acouf)

        if source_modal=="text":
            #テキストモダリティをソースモダリティに指定
            ...
            t_prob, a_prob, v_prob = model(textf, visuf_zero, acouf_zero, umask, qmask, lengths, demo_charaID_flag)
        elif source_modal=="audio":
            #オーディオモダリティをソースモダリティに指定
            log_prob1, log_prob2, log_prob3, all_log_prob, all_prob, \
            kl_log_prob1, kl_log_prob2, kl_log_prob3, kl_all_prob, \
            ws_t_prob, ws_a_prob, ws_v_prob, ws_all_prob, \
            t_prob, a_prob, v_prob = model(textf_zero, visuf_zero, acouf, umask, qmask, lengths, demo_charaID_flag)
        else:
            #視覚モダリティをソースモダリティに指定
            log_prob1, log_prob2, log_prob3, all_log_prob, all_prob, \
            kl_log_prob1, kl_log_prob2, kl_log_prob3, kl_all_prob, \
            ws_t_prob, ws_a_prob, ws_v_prob, ws_all_prob, \
            t_prob, a_prob, v_prob = model(textf_zero, visuf, acouf_zero, umask, qmask, lengths, demo_charaID_flag)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source_modal', type=str, default='text') #ソースモダリティを指定
    ...
    if args.model == "multi":
        ...
        test_pred_t, test_pred_a, test_pred_v= train_or_eval_model(model, loss_function, kl_loss, ws_loss, test_loader, e, data_flag=args.Dataset, source_modal=args.source_modal)
    ...
    #単一モダリティでの学習
        ...
        #検証，テスト(テストデータを使用)
        test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, misclassified, all_pred,\
            = uni_train_or_eval_model(model, loss_function, test_loader, data_flag=args.Dataset, modal=args.model)
